...
function initializeModels(attr, item) {
    ...
    function onChange(item, attr, data) {
        var attribute = item.getAttribute(attr);
    ...
    if (attr === 'es-show') {
        ...
        if (bindableFound) {
            this.data[bindableFound.key].some((dataItem) => {
                if (dataItem.$id === itemId) {
                    showItemData = Utils.getDataByKey(dataItem, attribute);
                    return true;
                }
            });
        }
        ...
        $(item).on('keyup', Utils.curry(onChange, item, attr, this.data));
                ...
                default:
                    ...
                    if (bindableFound) {
                        component.data[bindableFound.key].some((dataItem) => {
                            if (dataItem.$id === itemAttr) {
                                params.push(Utils.getDataByKey(dataItem, parameter));
                                return true;
                            }
                        });
                    }
...
function bindChanges(self, key, change) {
    ...
    self.bindables.forEach((bindable) => {
        ...
        var isInKeys = bindable.keys && bindable.keys.has(change.name);
        var isInBindableKey = bindable.key === change.name;
        var isInKey = bindable.key === key;

        if (isInKeys || isInBindableKey || isInKey) {
...
function observableChanged(self, key, changes) {
...
function observe(self, data) {
    Object.keys(data).forEach(function (key) {
        if (Object.prototype.toString.call(data[key]) === '[object Array]') {
            Object.observe(data[key], Utils.curry(observableChanged, self, key));
        }
    });

    Object.observe(data, Utils.curry(observableChanged, self, null));
}
