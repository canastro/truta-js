...
class DOM {
    ...
    find(selector) {
        ...
        //If you use [attribute=value] notation then should look for attribute
        if(selector.startsWith('[') && selector.endsWith(']')) {
            selector = selector.substring(1, selector.length - 1);
            values = selector.split('=');
            elements = this[0].querySelectorAll('*');

            found = Array.from(elements).some(function (item) {
                foundElement = item;
                return item.getAttribute(values[0]) === values[1];
            });
