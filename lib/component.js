...
function findDataItem(itemId, key, attribute) {
    ...
    Array.from(this.data[key]).some((dataItem) => {
...
function findBindable(key) {
    var found = false;
    var bindable;

    if (!this.bindables || !this.bindables.length) {
        return;
    }

    found = Array.from(this.bindables).some((item) => {
        bindable = item;
        return item.itemKey === key || item.keys.has(key);
    });

    return found ? bindable : null;
}

function setShowModel(attribute, item) {
    var itemId = item.getAttribute('es-id');
    var bindableKey = attribute.split('.')[0];
    var bindableFound = findBindable.call(this, bindableKey);
    var found = false;
    var showItemData;

    if (bindableFound) {
        bindableFound.key = bindableFound.key || bindableKey;
        showItemData = findDataItem.call(this, itemId, bindableFound.key, attribute);
    }
...
function calculateClassBindable(bindable) {
    ...
    //check if is inside a ng-repeat, atm the only way of doing that is by querying all the bindables for one
    //with findBindable function
    var key = findBindable.call(this, bindable.key);
    key = key ? key.key : bindable.key;
...
function setClassModel(attribute, item) {
    ...
    while ((m = re.exec(attribute)) !== null) {
        ...
        this.bindableClasses.push(bindable);
...
function getItemData(item, attribute) {
    ...
    if (bindableFound) {
        result = findDataItem.call(this, itemId, bindableFound.key, attribute);
    }
            ...
            switch (parameter) {
                ...
                default:
                    ...
                    if (bindableFound) {
                        ...
                        dataItem = findDataItem.call(component, itemId, bindableFound.key, parameter);
...
function bindChanges(self, change) {
    ...
    self.bindableClasses.forEach((bindable) => {
        if (bindable.key === change.name) {
            bindables.push(bindable);
        }
    });
