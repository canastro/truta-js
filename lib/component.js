...
function getItemData(item, attribute) {
    ...
    if (bindableFound) {
        data = findDataItem.call(this, itemId, bindableFound, attribute);
    }
...
function initializeModel(attribute, item, onChange) {
    ...
    var changeEvent = inputs[item.getAttribute('type') || ''].change;
    var inputValue = inputs[item.getAttribute('type') || ''].value;

    $(item).on(changeEvent, Utils.curry(onChange, this, item, attribute, this.data));

    var data = getItemData.call(this, item, attribute);
    item[inputValue] = data[attribute];
...
function initializeModels(attr, item) {
    ...
    function onChange(component, item, attr, data) {
        ...
        var inputValue = inputs[item.getAttribute('type') || ''].value;
        var data = getItemData.call(component, item, attr);
        ...
        tmp.forEach((parameter) => {
            switch (parameter) {
                ...
                default:
                    ...
                    if (bindableFound) {
                        Array.from(component.data[bindableFound.key]).some((dataItem) => {
...
export class Component {
    ...
    initialize(params) {
        ...
        this.data.addFisherman(observe.call(this));
